...
grad_rates_good <- grad_rates_la %>% filter(county_code %in% c("Los Angeles", 'Venture',
'Orange','San Bernardino', "Kern"))

grad_rates_bad <-  grad_rates_la %>% filter(county_code %in% c("Montgomery"))
```
...
```{r}
ap_la<- df_la %>%
  filter(county_code== "Los Angeles")
la_wealthy <- ap_la%>%
